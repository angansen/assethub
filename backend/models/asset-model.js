...
module.exports = class Asset {
    constructor(assetId, title, description, customer, createdBy,
    ...
    save(type) {
        ...
        return new Promise((resolve, reject) => {
            ...
            WIN_LESSONS_LEARNED=:WIN_LESSONS_LEARNED WHERE ASSET_ID=:ASSET_ID`,
                            [self.title, self.description, self.customer, self.createdBy.toLowerCase(),
                            self.serviceid, new Date(), self.modifiedBy, self.expiryDate, self.video_link, self.owner.toLowerCase(), assetState, self.assetId,
                ...
                :WIN_SALES_PROCESS_TEAMS,
                :WIN_LESSONS_LEARNED)`,
                            [assetid, self.title, self.description, self.customer, self.createdBy.toLowerCase(),
                                self.createdDate, self.serviceid, self.thumbnail, self.modifiedDate, self.modifiedBy,
                                self.ASSET_VIDEO_URL, self.expiryDate, self.video_link, self.owner.toLowerCase(), assetState,
                                self.windata.WIN_ECA,
                                self.windata.WIN_REGID,
                                self.windata.WIN_FISCAL_YR,
                                self.windata.WIN_MEMBERS,
    ...
    saveTest() {
        ...
        ASSET_STATUS=:ASSET_STATUS 
             WHERE ASSET_ID=:ASSET_ID`,
                            [self.title, self.description, self.description, self.customer, self.createdBy.toLowerCase(),
                            self.serviceid, new Date(), self.modifiedBy, self.expiryDate, self.video_link, self.owner.toLowerCase(), 'Pending Review', self.assetId],
                            {
                                outFormat: oracledb.Object
                            }).then(res => {
                                console.log('1st update done(Asset details updated)')
                ...
                connection.transaction([
                    function firstAction() {
                        return connection.insert(`INSERT into ASSET_DETAILS(ASSET_ID,ASSET_TITLE,ASSET_DESCRIPTION,ASSET_CUSTOMER,ASSET_CREATEDBY,ASSET_CREATED_DATE,ASSET_SERVICE_ID,
                ASSET_THUMBNAIL,ASSET_MODIFIED_DATE,ASSET_MODIFIED_BY,ASSET_VIDEO_URL,ASSET_EXPIRY_DATE,ASSET_VIDEO_LINK,ASSET_OWNER,ASSET_STATUS) values(:ASSET_ID,:ASSET_TITLE,:ASSET_DESCRIPTION,
                :ASSET_CUSTOMER,:ASSET_CREATEDBY,:CREATED_DATE,:ASSET_SERVICE_ID,
                :ASSET_THUMBNAIL,:ASSET_MODIFIED_DATE,:ASSET_MODIFIED_BY,:ASSET_VIDEO_URL,:ASSET_EXPIRY_DATE,:ASSET_VIDEO_LINK,:ASSET_OWNER,:ASSET_STATUS)`,
                            [assetid, self.title, self.description, self.customer, self.createdBy.toLowerCase(),
                                self.createdDate, self.serviceid, self.thumbnail, self.modifiedDate, self.modifiedBy, self.ASSET_VIDEO_URL, self.expiryDate, self.video_link, self.owner.toLowerCase(), 'Pending Review'],
                            {
                                outFormat: oracledb.Object
                            }).then(res => {
                                console.log('1st insert done(Asset details inserted)')
